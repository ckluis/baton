...
Two of the four gates reach a person: the **blocked batch** (§8.3) and the **final gate**
(§8.4). Each one ships, beside its written file, one HTML page written for that person:

| gate | brief | written from |
|---|---|---|
| blocked batch | `_orch/brief/blocked-<n>.html` | the batch's `_orch/inbox/Q-*.md` files |
...
`_orch/`. A brief that says something the record does not say is a defect in the brief. It is
temporary by construction: it lives under `_orch/`, is gitignored, and is disposed of with the
run.

**The shape is fixed — 1-3-1.** A title. One paragraph, at most eighty words, on what the run
was asked to do and what it did. Then:

1. **One problem.** The single decision this page exists to put in front of the reader, in one
   sentence. A final brief with no decision states that plainly and the three options below are
   about what to do with the result.
2. **Three options.** Exactly three. Each states what it is, what it costs, what it risks, and
   what it settles. Fewer than three means the run has not looked; more is a menu, and a menu is
   work handed back. When only one option is real, the other two are *do nothing* — the default
